...
const { encrypt, decrypt, isEncryptedEnvelope } = require('./crypto-utils');
const fs   = require('fs');
...
function saveGuildSessions(guildId) {
  ...
  try {
    ...
    const json = JSON.stringify([...sessions.entries()]);
    fs.writeFileSync(path.join(dir, 'sessions.json'), encrypt(json));
  } catch (err) {
...
function saveGuildConfig(guildId) {
  ...
  try {
    const dir = getGuildDir(guildId);
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, 'config.json'), encrypt(JSON.stringify(config)));
  } catch (err) {
...
function loadGuildConfig(guildId) {
  try {
    ...
    const config = JSON.parse(wasEncrypted ? decrypt(raw) : raw);
    guildConfigs.set(guildId, config);
    if (!wasEncrypted) saveGuildConfig(guildId); // migrate legacy plaintext file to encrypted format
    return config;
  } catch (err) {
...
function loadGuildSessions(guildId) {
  try {
    ...
    const raw = fs.readFileSync(file, 'utf8');
    const wasEncrypted = isEncryptedEnvelope(raw);
    const data = JSON.parse(wasEncrypted ? decrypt(raw) : raw);
    const map  = new Map();
    ...
    if (!wasEncrypted) saveGuildSessions(guildId); // migrate legacy plaintext file to encrypted format
  } catch (err) {
